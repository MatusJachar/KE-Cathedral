...
def _get_cached(k):
    e = _cache.get(k)
    return e["data"] if e and _time.time() - e["ts"] < 300 else None

def _set_cache(k, d):
    _cache[k] = {"data": d, "ts": _time.time()}

def _valid_uuid(v):
    return bool(_re.match(r"^[0-9a-f-]{36}$", str(v)))

def _sanitize(v, m=500):
    cleaned = _re.sub(r"[<>&%;()]", "", str(v))
    return cleaned[:m].strip()
...
# ==================== STRIPE PAYMENTS ====================
# import stripe  # disabled - install via pip
...
app.add_middleware(
    ...
    allow_origins=["exp://","http://localhost:8081","http://localhost:19006"],
    allow_methods=["*"],
    allow_headers=["*"],
)
